...
void check_range(char* name, double *x, int size, double lower, double upper){
  for(int i=0; i<size; i++){
    if(x[i] > upper){ error("%s[%d] = %f > %f\n", name, i, x[i], upper); }
    if(x[i] < lower){ error("%s[%d]  = %f < %f\n", name, i, x[i], lower); }
    if(ISNA(x[i])){ error("%s[%d] is NA\n", name, i); }
    if(ISNAN(x[i])){ error("%s[%d] is NaN\n", name, i); }
  }
}
...
void MCEM_EStep_arscid(
  ...
  generateObsIndex2(obsIndex_user, oiStart_user, oiNum_user, user, nObs, nUsers, debug);
  generateObsIndex2(obsIndex_item, oiStart_item, oiNum_item, item, nObs, nItems, debug);
  ...
  for(s=0; s<(*nSamples+*nBurnin); s++){
    ...
    if(*debug >= 10) check_range("alpha", alpha, *nUsers, *ars_xl, *ars_xu);
    //center
    ...
    if(*debug >= 10) check_range("beta", beta, *nItems, *ars_xl, *ars_xu);
    ...
    if(*main_effects==0){
      ...
      if(*debug >= 10) check_range("u", u, (*nUsers)*(*nFactors), *ars_xl, *ars_xu);
      ...
      if(*debug >= 10) check_range("v", v, (*nItems)*(*nFactors), *ars_xl, *ars_xu);
  ...
  if(*debug >= 10 && *center == 0){
    Rprintf("Check factor ranges\n");
    check_range("alpha_mean", alpha_mean, *nUsers, *ars_xl, *ars_xu);
    check_range("beta_mean", beta_mean, *nItems, *ars_xl, *ars_xu);
    check_range("u_mean", u_mean, (*nUsers)*(*nFactors), *ars_xl, *ars_xu);
    check_range("v_mean", v_mean, (*nItems)*(*nFactors), *ars_xl, *ars_xu);
  }
 ...
 */
SEXP MCEM_EStep_arscid_Call(
  // OUTPUT
  SEXP o_mean /*nObs x 1*/,
  SEXP alpha_mean/*nUsers x 1*/,    SEXP alpha_sumvar/*1x1*/,
  SEXP beta_mean/*nItems x 1*/,     SEXP beta_sumvar/*1x1*/,
  SEXP u_mean/*nUsers x nFactors*/, SEXP u_sumvar/*nFactors x 1*/,
  SEXP v_mean/*nItems x nFactors*/, SEXP v_sumvar/*nFactors x 1*/,
  SEXP ars_XI_alpha, SEXP ars_XI_beta,
  SEXP ars_XI_u,     SEXP ars_XI_v,
  // INPUT
  SEXP nSamples, SEXP nBurnin,
  SEXP user/*nObs x 1*/,        SEXP item/*nObs x 1*/,
  SEXP y/*nObs x 1*/,           SEXP xb/*nObs x 1*/,
  SEXP g0w/*nUsers x 1*/,       SEXP d0z/*nItems x 1*/,
  SEXP Gw/*nUsers x nFactors*/, SEXP Dz/*nItems x nFactors*/,
  SEXP alpha_in/*nUsers x 1*/,    SEXP beta_in/*nItems x 1*/,
  SEXP u_in/*nUsers x nFactors*/, SEXP v_in/*nItems x nFactors*/,
  SEXP var_alpha, SEXP var_beta,
  SEXP var_u, SEXP var_v, /*nFactors x 1*/
  SEXP nObs, SEXP nUsers, SEXP nItems, SEXP nFactors,
  SEXP ars_ninit, SEXP ars_qcent, SEXP ars_xl, SEXP ars_xu, SEXP ars_alpha,
  // OTHER
  SEXP debug, SEXP main_effects, SEXP beta_int, SEXP center
){
  MCEM_EStep_arscid(
	  // OUTPUT
	  MY_REAL(o_mean),
	  MY_REAL(alpha_mean),   MY_REAL(alpha_sumvar),
	  MY_REAL(beta_mean),    MY_REAL(beta_sumvar),
	  MY_REAL(u_mean),       MY_REAL(u_sumvar),
	  MY_REAL(v_mean),       MY_REAL(v_sumvar),
	  MY_REAL(ars_XI_alpha), MY_REAL(ars_XI_beta),
	  MY_REAL(ars_XI_u),     MY_REAL(ars_XI_v),
	  // INPUT
	  MY_INTEGER(nSamples), MY_INTEGER(nBurnin),
	  MY_INTEGER(user),     MY_INTEGER(item),
	  MY_REAL(y),           MY_REAL(xb),
	  MY_REAL(g0w),         MY_REAL(d0z),
	  MY_REAL(Gw),          MY_REAL(Dz),
	  MY_REAL(alpha_in),    MY_REAL(beta_in),
	  MY_REAL(u_in),        MY_REAL(v_in),
	  MY_REAL(var_alpha),   MY_REAL(var_beta),
	  MY_REAL(var_u),       MY_REAL(var_v),
	  MY_INTEGER(nObs), MY_INTEGER(nUsers), MY_INTEGER(nItems), MY_INTEGER(nFactors),
	  MY_INTEGER(ars_ninit), MY_REAL(ars_qcent), MY_REAL(ars_xl), MY_REAL(ars_xu), MY_REAL(ars_alpha),
	  // OTHER
	  MY_INTEGER(debug), MY_INTEGER(main_effects), MY_INTEGER(beta_int), MY_INTEGER(center)
  );
  return R_NilValue;
}
